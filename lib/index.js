...
const LIMITS = {
  ...
  codeBlockMultiplier: { min: 1, max: 100 },
}
...
function createStreamGuard(options, runtime) {
  ...
  function feedText(b, delta) {
    b.text += delta
    const codeThreshold = Math.max(runtime.threshold, runtime.threshold * runtime.codeBlockMultiplier)
    const runs = runtime.skipCodeBlocks ? b.fence.push(delta) : [{ text: delta, code: false }]
...
/** 代码块内实际使用的最严格阈值（未开启代码块放宽时等于 threshold）。 */
function effectiveCodeThreshold(runtime) {
  if (runtime.skipCodeBlocks !== true) return runtime.threshold
  return runtime.threshold * runtime.codeBlockMultiplier
}
