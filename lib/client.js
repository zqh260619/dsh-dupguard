...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      saveFailed: '保存失败：宿主未接受该值，请查看 DSH 宿主日志。',
    ...
    const en = {
      ...
      saveFailed: 'Save failed: the host did not accept the value; check the DSH host log.',
    ...
    function DupguardSection(props) {
      ...
      // UI 同步模型：手动修改前跟随镜像快照（含本页挂载时的强制重拉），
      // 手动修改后本地自治（镜像回传不再覆盖，避免旧值回滚）。
      const remoteList = Array.isArray(snap.value && snap.value.ignoredChars) ? snap.value.ignoredChars : []
      const [list, setList] = React.useState(null)
      const touched = React.useRef(false)
      const lastRemote = React.useRef(remoteList)
      if (list === null || (!touched.current && lastRemote.current !== remoteList)) {
        lastRemote.current = remoteList
        setList(remoteList)
      }
      ...
      // 写路径走 settingsScope 控制器（DSH 自己的设置写通道）：自动携带最新
      // revision、串行化并发写，并把宿主应答折叠回共享镜像。
      // 注意：DSH 0.1.2 起客户端不再暴露 connection.api（改为 ctx.remote.<ns>），
      // 直接调 wire 面会在 0.1.2 上抛错并卡在「保存中…」；控制器接口两版一致。
      const finishWrite = (expected) => {
        // 写入结果由控制器折叠进镜像，回读快照校验是否真的落盘。
        const snapNow = props.controller.getSnapshot()
        const applied = snapNow !== undefined && snapNow.value !== undefined && Array.isArray(snapNow.value.ignoredChars)
          ? snapNow.value.ignoredChars
          : null
        const same = applied !== null && applied.length === expected.length &&
          applied.every((ch, index) => ch === expected[index])
        setWriteState(same ? 'saved' : 'error:' + t('saveFailed'))
      }
      const runWrite = (operation, expected) => {
        setWriteState('saving')
        Promise.resolve()
          .then(() => operation())
          .then(() => finishWrite(expected), (error) => {
            setWriteState('error:' + String((error && error.message) || error))
          })
      }
      const persist = (next) => {
        runWrite(() => props.controller.set('ignoredChars', next), next)
      }
      const persistUnset = () => {
        // unset 后解析值回到 schema 默认（宿主 base）白名单。
        runWrite(() => props.controller.unset('ignoredChars'), ['-', '|'])
      }
      ...
      return React.createElement('div', { className: 'dg-section' },
        ...
        writeState === null
          ...
          : writeState === 'saving'
            ? React.createElement('p', { className: 'dg-note' }, '保存中…')
            : writeState === 'saved'
              ? React.createElement('p', { className: 'dg-note' }, '已保存')
              : React.createElement('p', { className: 'dg-note', style: { color: 'var(--dsw-alias-state-error-primary)' } }, writeState),
      )
    }
    ...
    function apply(ctx) {
      ...
      // （{ignoredChars: [...]}），而非 wire 视图。直接解读该值的 ignoredChars 字段。
