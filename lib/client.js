  ...
  factory: (require) => {
    ...
    function DupguardSection(props) {
      ...
      const runWrite = (operation, verify) => {
        ...
        // 失败时带上宿主返回的原始原因（writeReason），否则只剩泛化的「保存失败」无从定位。
        const failureText = () => {
          const reason = typeof props.writeReason === 'function' ? props.writeReason() : null
          return reason === null || reason === undefined ? t('saveFailed') : t('saveFailed') + '：' + String(reason)
        }
        Promise.resolve()
          .then(() => operation())
          .then(() => setWriteState(verify() ? 'saved' : 'error:' + failureText()), (error) => {
