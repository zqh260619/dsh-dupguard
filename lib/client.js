...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function pickNamespace(view) {
      ...
      // 兜底：entry id 随安装方式变化（bundle / 手工插入 / 别名）时按字段签名识别。
    ...
    function createSettingsBridge() {
      ...
      let mirror = { status: 'idle', error: null }
      ...
      function remoteChannel(remote) {
        ...
        const acceptView = (view) => {
          if (view !== null && typeof view === 'object' && view.writable === false) loopback = false
          const row = rowOfView(view)
          if (row === undefined) {
            publish({ status: 'unavailable', value: undefined, user: undefined }, { status: 'ready', error: null })
            return
          }
          ...
          try {
            ...
            if (response === null || typeof response !== 'object' || response.ok !== true) {
              publish(
                ...
                { status: 'error', error: responseError(response, 'settings.describe failed') },
              )
              return
            }
      ...
      function attach(kind, factory, service) {
        ...
        return () => {
          ...
          channel = null
          channelKind = null
          publish({ status: 'loading', value: undefined, user: undefined }, { status: 'idle', error: null })
        }
      }
    ...
    function apply(ctx) {
      ...
      ctx.inject(['remote.settings'], (scope) => {
        ...
        if (remote === undefined || remote.settings === undefined) return () => {}
        if (remote.$host !== undefined && typeof remote.$host.isLoopback === 'boolean') {
          bridge.setLoopback(remote.$host.isLoopback)
        }
        const detach = bridge.attachRemote(remote)
        const disposers = []
        // 设置文档变化 / 连接重置时重拉，保持面板与宿主持久化一致。
      ...
      ctx.inject(['settingsScope'], (scope) => {
        ...
        return bridge.attachLegacy(settingsScope)
      })
