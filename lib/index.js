...
/** 设置 schema：白名单字符数组（其余检测参数保持代码常量）。 */
const SETTINGS_SCHEMA = z.object({
  ignoredChars: z.array(z.string()).default(['-', '|']),
})
...
function createStreamGuard(options, runtime) {
  ...
  function feedText(b, delta) {
    b.text += delta
    const piece = sanitizeDelta(delta, runtime)
    b.stripped = (b.stripped + piece).slice(-runtime.detectionWindow)
    return findRepeatedTail(b.stripped, runtime.threshold, runtime.minUnitLength, runtime.maxUnitLength)
  }
...
/**
 * DSH ≤ 0.1.1-rc.1 standing-mount 兼容补丁：把 cordisInspect.register 幂等化。
 *
 ...
 * cordisInspect.providers 为可读 Map（实测 rc.6 / rc.7 / 0.1.1-rc.1 如此）。
...
/**
 * 把 CONFIG 拷贝为运行时配置：ignoredChars 会被设置页热更新，
 * 其余字段保持代码常量（设置页只暴露白名单）。
 */
...
/**
 * 从 DSH 的 settings 服务注册白名单设置（仅常驻版）。
...
function installSettings(ctx, runtime) {
  ...
  try {
    const scope = settings.register(SETTINGS_NS, SETTINGS_SCHEMA, {
      base: { ignoredChars: CONFIG.ignoredChars },
    })
    const sync = () => {
      runtime.ignoredChars = [...scope.get().ignoredChars]
    }
    sync()
    const stop = scope.watch(sync)
    ctx.effect(() => () => stop(), 'dupguard: settings watch')
    console.log('[dupguard] 已注册设置命名空间 ' + SETTINGS_NS + '（白名单可在设置页修改）')
  } catch (error) {
    // 设置注册失败不得破坏检测主体：白名单保持代码常量，仅损失设置页能力。
    console.error('[dupguard] 设置命名空间注册失败（白名单将使用默认值）：' + (error && error.message ? error.message : String(error)))
  }
}

function apply(ctx) {
  // 启动自检：把关键服务可见性直接打到宿主日志，便于诊断挂载问题。
  const settingsProbe = ctx.get('settings')
  const inspectProbe = ctx.get('cordisInspect')
  console.log('[dupguard] 常驻插件 apply：settings=' + (settingsProbe === undefined ? 'missing' : 'present') + ' cordisInspect=' + (inspectProbe === undefined ? 'missing' : 'present'))
