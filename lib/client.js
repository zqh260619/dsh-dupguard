...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      list: 'Ignored characters (whitelist)',
    ...
    function DupguardSection(props) {
      ...
      const fieldError = (field) => {
        ...
        if (field.key === 'maxUnitLength' && crossInvalid) return fmt(t('errCross'), { min: parsed.minUnitLength })
        return null
      }
      ...
      const add = () => {
        ...
        setDraft('')
        if (shown.ignoredChars.indexOf(ch) !== -1) return
        commitList([...shown.ignoredChars, ch])
      }
