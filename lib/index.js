 ...
 * 凑满阈值，因此 p 超过 floor(窗口 / 阈值) 的重复单元无法被识别。返回缺口描述
 * （含所需窗口）或 null。
 */
function windowShortfall(runtime) {
  const needed = runtime.threshold * runtime.maxUnitLength
  if (runtime.detectionWindow >= needed) return null
  return {
    needed: needed,
    current: runtime.detectionWindow,
    effectiveMaxUnit: Math.floor(runtime.detectionWindow / runtime.threshold),
  }
}
...
function warnShortWindow(runtime) {
  ...
  console.warn(
    ...
    '（= 阈值 ' + String(runtime.threshold) + ' × 最大单元长度 ' + String(runtime.maxUnitLength) +
    '），当前 ' + String(short.current) + '；超过 ' + String(short.effectiveMaxUnit) +
    ' 字符的重复单元将无法识别。请在设置页提高「检测窗口」，或降低阈值/最大单元长度。'
  )
}
...
function applySettingsToRuntime(runtime, value) {
  ...
  runtime.ignoredChars = Array.isArray(source.ignoredChars)
    ? source.ignoredChars.filter((ch) => typeof ch === 'string')
    : [...fallback.ignoredChars]
