...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DupguardSection(props) {
      ...
      // 直接经设置 RPC 写（models 页同款），可观察到真实错误；
      // 乐观更新保证界面即时反映，写入结果异步回填状态。
      const persist = (next) => {
        setWriteState('saving')
        const revision = snap.revision === undefined ? undefined : snap.revision
        props.api.settings.mutate({
          ns: SETTINGS_NS,
          ops: [{ op: 'set', path: ['ignoredChars'], value: next }],
          ...(revision === undefined ? {} : { expectedRevision: revision }),
        }).then((response) => {
          if (!response || !response.result || !response.result.ok) {
            const message = response && response.result && response.result.error ? response.result.error.message : 'unknown'
            setWriteState('error:' + message)
          } else {
            setWriteState('saved')
            // 写成功后重拉 describe 刷新镜像，保证重开设置页显示持久化值。
            if (props.mirror && typeof props.mirror.load === 'function') {
              props.mirror.load()
            }
          }
      ...
      const persistUnset = () => {
        ...
        props.api.settings.mutate({
          ns: SETTINGS_NS,
          ops: [{ op: 'unset', path: ['ignoredChars'] }],
          ...(revision === undefined ? {} : { expectedRevision: revision }),
        }).then((response) => {
          if (!response || !response.result || !response.result.ok) {
            const message = response && response.result && response.result.error ? response.result.error.message : 'unknown'
    ...
    function apply(ctx) {
      ...
      const injected = () => ({ controller, t, isLoopback, mirror, api: connection.api })
