...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      codeBlockMultiplierHint: '围栏代码块（``` / ~~~）内的重复按「阈值 × 本倍数」判定。范围 {min}–{max}；越大越不容易误杀正常代码，但也越晚兜住代码块里的失控复读（1 = 不放宽）。',
    ...
    const en = {
      ...
      codeBlockMultiplierHint: 'Inside fenced code blocks (``` / ~~~) a repeat must reach threshold x this multiplier. Range {min}–{max}; higher means fewer false stops on generated code, but runaway loops inside code are caught later (1 = no relaxation).',
    ...
    function DupguardSection(props) {
      ...
      // 代码块内使用放宽阈值（阈值 × 倍数），因此窗口需按更严格的一侧计算。
      const strictThreshold = shown.skipCodeBlocks === true
        ? parsed.threshold * parsed.codeBlockMultiplier
        : parsed.threshold
      const shortfall = numericValid && !crossInvalid && parsed.detectionWindow < strictThreshold * parsed.maxUnitLength
        ? {
            ...
            multiplier: shown.skipCodeBlocks === true ? parsed.codeBlockMultiplier : 1,
