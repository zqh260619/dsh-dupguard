...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // 自定义 decode：绕开客户端 schema 复水化与校验，直接解读 wire 视图的
      // ignoredChars 字段。字段形状由 host 端的 SETTINGS_SCHEMA 保证。
      const controller = ctx.settingsScope.bind({
        namespace: SETTINGS_NS,
        decode: (raw) => {
          if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) return undefined
          const list = raw.ignoredChars
          if (!Array.isArray(list)) return undefined
          if (!list.every((ch) => typeof ch === 'string')) return undefined
          return { ignoredChars: [...list] }
        },
      })
      ctx.effect(() => () => {
        controller.dispose()
      }, 'dupguard: settings scope')
      const injected = () => ({ controller, t, isLoopback })
